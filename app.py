...
\
# ================= Streamlit Config =================\
st.set_page_config(page_title="Swing Trading + Fundamentals Dashboard", page_icon="\uc0\u55357 \u56522 ", layout="wide")\
st.title("\uc0\u55357 \u56522  Swing Trading + Fundamentals Dashboard")\
\
# ---------------- Sidebar: Developer + Settings ----------------\
with st.sidebar:\
    st.markdown("### \uc0\u55357 \u56424 \u8205 \u55357 \u56507  Developer Info")\
    st.markdown("**Mr. Shivam Maheshwari**")\
    st.write("\uc0\u55357 \u56599  [LinkedIn](https://www.linkedin.com/in/theshivammaheshwari)")\
    st.write("\uc0\u55357 \u56568  [Instagram](https://www.instagram.com/theshivammaheshwari)")\
    st.write("\uc0\u55357 \u56536  [Facebook](https://www.facebook.com/theshivammaheshwari)")\
    st.write("\uc0\u9993 \u65039  247shivam@gmail.com")\
    st.write("\uc0\u55357 \u56561  +91-9468955596")\
    st.markdown("---")\
...
try:\
    ...
    pass  # if not available, we\'92ll use text input\
\
# ================= UI: Input section =================\
col_in1, col_in2 = st.columns([2,1])\
with col_in1:\
    if all_stock_codes:\
        user_input = st.selectbox("\uc0\u55357 \u56589  Search or select stock symbol:", all_stock_codes, index=all_stock_codes.index("RELIANCE") if "RELIANCE" in all_stock_codes else 0)\
    else:\
        user_input = st.text_input("Enter stock symbol (e.g., RELIANCE, TCS, INFY, AAPL):", value="RELIANCE")\
\
with col_in2:\
    run_btn = st.button("Analyze \uc0\u55357 \u56960 ", use_container_width=True)\
\
if run_btn:\
...
\
    st.markdown(f"### \uc0\u55357 \u56520  Swing Trading Analysis - \{company_name\} (\{user_input\})")\
...
\
    if techs and hist is not None:\
        ...
        st.subheader("\uc0\u55357 \u56590  Key Trade Highlights")\
...
\
        with c2:\
            ...
            if not fib_df.empty:\
                st.markdown("#### \uc0\u55356 \u57263  Fibonacci Targets")\
                st.table(fib_df.style.hide(axis="index"))\
\
        # -------- Detailed Technicals --------\
        st.subheader("\uc0\u55357 \u56522  Detailed Technicals")\
...
\
        ...
        st.error("\uc0\u10060  No technical data found. Tried: " + ", ".join([t for t in (tried or []) if t]))\
\
    # -------- Fundamentals (yfinance) --------\
    st.markdown(f"### \uc0\u55356 \u57318  Fundamentals - \{company_name\} (\{user_input\})")\
    if funds:\
        ...
        if flags:\
            st.write("Highlights: " + " \'95 ".join(flags))\
\
...
\
    ...
    st.markdown(f"### \uc0\u55357 \u56516  Screener.in Snapshot - \{company_name\} (\{user_input\})")\
...
\
    ...
    st.info("Select a symbol and click Analyze \uc0\u55357 \u56960 ")\
}